...
Preset **standard**, N=20000, repetitions=150, seed=84. Thí nghiệm xác suất thấy lỗi dùng cỡ khối/số lần lặp riêng theo metadata.
...
Ý nghĩa thống kê: N=20,000; R=150; mean +/- 1 SEM across runs; dashed = theory; sample fraction=0.20; pZ=0.50.
...
Ý nghĩa thống kê: N=20,000; R=150; mean +/- 1 SEM; no Eve, no loss; sample fraction=0.20; pZ=0.50.
...
Ý nghĩa thống kê: N=20,000; R=150; LEFT bars = 1 SD (not SEM); CENTER = mean per-run Wilson width; no finite-key security claim.
...
Ý nghĩa thống kê: R=2000 per point; N=max(512,8m); bars = 95% Wilson across complete runs; dashed = theory; no noise/loss.
...
Ý nghĩa thống kê: N=20,000; R=150; mean +/- 1 SEM; dashed = theory; candidate curve approximate due to floor(0.2*sifted).
...
```text
python -m bb84 experiments --preset standard --output results-reproduced --seed 84 --signals 20000 --repetitions 150
```
